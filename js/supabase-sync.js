...
// Pull store from Supabase.
// Returns:
//   { ok: true,  data: <store object> }  — row found
//   { ok: true,  data: null }            — reached server, user has no row yet (new user)
//   { ok: false }                        — could NOT reach/read the server (error or timeout)
async function sbPullStore() {
  console.log('[sbPullStore] Starting pull for user:', _sbUser?.email);
  if (!_sb || !_sbUser) return { ok: false };
  ...
  try {
    ...
    if (!token) { console.warn('[sbPullStore] No access token'); return { ok: false }; }
    ...
    if (!response.ok) {
      ...
      return { ok: false };           // server errored — failure, do NOT arm saving
    }

    const data = await response.json();
    console.log('[sbPullStore] REST API result:', data?.length > 0 ? 'found' : 'empty (no row)');

    if (data && data.length > 0) return { ok: true, data: data[0].data };
    return { ok: true, data: null };  // reached server, genuinely no data yet
  } catch(e) {
    console.warn('[sbPullStore] Exception:', e.name === 'AbortError' ? 'Timeout' : e.message);
    return { ok: false };             // timeout / network error — failure
  }
}
